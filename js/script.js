...
let intervalId = null; /* Id del intervalo tiempo */
...
function toggleCell(i, j) {
    board[i][j].classList.toggle('live'); /* Aplicar todos los cambios después de calcular el nuevo estado del tablero */
}
function starGame() {
...
function updateBoard() {
    let changes = []; /* almacenar los cambios para aplicativos despues de calcular todos los estados */
...
function calculateBoardStatus(changes) {
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < rows; j++) {
            ...
            if (isLive && (liveCount < 2 || liveCount > 3)) {/* 1. Regla: Muere por soledad o por sobre población */
                changes.push({ i, j, live: false })
            } else if (!isLive && liveCount === 3) { /* 2. Regla: Nace una nueva célula */
